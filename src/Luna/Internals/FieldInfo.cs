...
public class FieldInfo {
	...
	public FieldInfo(Stream Stream, ConstantPool Pool) {
		AccessFlags = (FieldAccessFlags) Stream.ReadUShort();
		...
		for (ushort i = 0; i < Attributes.Length; i++) {
			Attributes[i] = AttributeInfo.Parse(Stream, Pool) ?? throw new ClassFileException($"Cannot parse an incomplete or otherwise malformed class file: ran out of bytes while reading the cpool index of attribute #{i} (owned by a FieldInfo).");
		}
	}

	public FieldInfo(FieldAccessFlags AccessFlags, ushort NameIndex, ushort DescriptorIndex, AttributeInfo[] Attributes) {
	...
	public override bool Equals(object? Object) => Object is FieldInfo CFI && CFI.AccessFlags == AccessFlags && CFI.DescriptorIndex == DescriptorIndex && CFI.NameIndex == NameIndex && CFI.Attributes.SequenceEqual(Attributes);
	public override string ToString() => $"{{ Field #{NameIndex}, #{DescriptorIndex} w/ Access 0x{AccessFlags:X}, Attributes [ {GU.ToString(Attributes)} ] }}";
